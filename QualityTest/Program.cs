...
namespace QualityTest {
    class Program {
        ...
        static void Main(string[] args) {
            ...
            List<GradeDigest> testDigests = LoadTestDigests();
            ...
            Util.Timed("compare with database", () => {
                ...
                foreach (var gd in testDigests) {
                    ...
                    gradePairs.Add(new Tuple<GradeDigest, RecognitionResult>(gd, digestSet.FindBestMatch(gd)));
            ...
            Func<int, int> percentTotal = c => (int) Math.Floor((double) c / gradePairs.Count * 100);
            Func<int, int, int> percent = (c, g) => {
                int tot = gradePairs.Count(gp => gp.Item1.grade == g);
                if (tot > 0) {
                    return (int) Math.Floor((double) c / tot * 100);
                } else {
                    return 0;
                }
            };
            ...
            foreach (double confidenceThreshold in new List<double> { 0.7, 0.6, 0.5 }) {
                ...
                Console.WriteLine("Total (r/w/u): {0}/{1}/{2}, {3}/{4}/{5} %",
                ...
                for (byte g = 2; g <= 5; g++) {
                    Console.WriteLine("{0}: {1}/{2}/{3}, {4}/{5}/{6} %", g,
        ...
        public static List<GradeDigest> LoadTestDigests() {
